...
import { insertIncidentSchema, insertLogSchema, updateProfileSchema, User, insertForumCategorySchema, insertForumPostSchema, insertForumReplySchema } from "@shared/schema";
import "./types";
...
export async function registerRoutes(
  ...
  // --- Lease Upload ---
  app.post("/api/upload/lease", (req, res, next) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    next();
  }, uploadLease.single("file"), async (req, res) => {
  ...
  app.delete("/api/logs/:id/cascade", async (req, res) => {
    ...
    const chatLogs = allLogs
      ...
      .sort((a: any, b: any) => {
        ...
        return a.id - b.id; // Use ID as tie-breaker for same timestamp
      });
  ...
  app.post("/api/incidents/:id/upload", (req, res, next) => {
    if (!req.isAuthenticated()) return res.sendStatus(401);
    next();
  }, uploadEvidence.single("file"), async (req, res) => {
    if (!req.file) return res.status(400).json({ error: "No file uploaded" });
