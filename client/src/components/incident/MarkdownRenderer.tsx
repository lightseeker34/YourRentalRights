...
export const MarkdownRenderer = memo(({ content, isAi = false }: MarkdownRendererProps) => {
  ...
  return (
    <div className="markdown-content w-full max-w-full min-w-0 break-words [overflow-wrap:anywhere] px-[10px] md:px-0" style={{ fontFamily: 'var(--font-chat)' }}>
      <ReactMarkdown 
        ...
        components={{
          ...
          table: ({children}) => (
            <div className="w-full max-w-full overflow-x-auto my-1 md:my-4 border rounded-lg shadow-sm bg-white block -mx-[10px] md:mx-0" style={{ maxWidth: 'calc(100% + 20px)' }}>
          ...
          th: ({children}) => <th className="px-2 md:px-4 py-0.5 md:py-1.5 font-semibold text-slate-700 whitespace-nowrap min-w-[100px]">{children}</th>,
          td: ({children}) => <td className="px-2 md:px-4 py-0.5 md:py-1.5 text-slate-600 whitespace-nowrap md:whitespace-pre-wrap min-w-[120px] max-w-[250px] md:max-w-[400px] overflow-hidden text-ellipsis md:overflow-visible">{children}</td>,
        }}
      >
        {content}
      </ReactMarkdown>
