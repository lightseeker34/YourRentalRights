  ...
  return (
    <div className="flex-shrink-0 w-full sm:w-64 pr-4" data-testid={`incident-card-${incident.id}`}>
      ...
      {groupedEntries().length > 0 && (
        <div className="ml-3 border-l-2 border-slate-200 pl-2 mt-2 space-y-2">
            ...
            if (item.type === 'chat_group') {
              ...
              return (
                <div key={item.id}>
                  {!isExpanded && (
                    <div 
                      className="bg-slate-50 border border-slate-200 rounded-lg p-2 shadow-sm hover:bg-slate-100 transition-colors cursor-pointer max-w-[86%] sm:max-w-[14rem] ml-auto"
                    ...
                    <div className="space-y-1">
                      ...
                      {item.chats.map((log) => (
                        <Link key={log.id} href={`/dashboard/incident/${incident.id}?log=${log.id}`}>
                          <div className={`border rounded-lg p-2 shadow-sm transition-colors cursor-pointer max-w-[86%] sm:max-w-[14rem] ml-auto ${
                            log.isAi 
            ...
            return (
              <div key={log.id} className="relative group/entry">
                <Link href={`/dashboard/incident/${incident.id}?log=${log.id}`}>
                  <div className="bg-slate-50 border border-slate-200 rounded-lg p-2 shadow-sm hover:bg-slate-100 transition-colors cursor-pointer max-w-[86%] sm:max-w-[14rem] ml-auto">
                    <div className="flex items-center justify-between gap-1.5 mb-1">
                      <div className="flex items-center gap-1.5 min-w-0">
