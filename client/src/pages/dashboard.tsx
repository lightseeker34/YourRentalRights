            ...
            if (item.type === 'chat_group') {
              ...
              return (
                <div key={item.id}>
                  {!isExpanded && (
                    <div 
                      className="bg-slate-50 border border-slate-200 rounded-lg p-2 sm:p-2.5 shadow-sm hover:bg-slate-100 transition-colors cursor-pointer w-full sm:max-w-[15.5rem]"
                    ...
                    <div className="space-y-1">
                      ...
                      {item.chats.map((log) => (
                        <Link key={log.id} href={`/dashboard/incident/${incident.id}?log=${log.id}`}>
                          <div className={`border rounded-lg p-2 sm:p-2.5 shadow-sm transition-colors cursor-pointer pl-[10px] pr-[10px] w-full sm:max-w-[15.5rem] ${
                            log.isAi 
            ...
            return (
              <div key={log.id} className="relative group/entry">
                <Link href={`/dashboard/incident/${incident.id}?log=${log.id}`}>
                  <div className="bg-slate-50 border border-slate-200 rounded-lg p-2 sm:p-2.5 shadow-sm hover:bg-slate-100 transition-colors cursor-pointer pl-[10px] pr-[10px] w-full sm:max-w-[15.5rem]">
                    <div className="flex items-center justify-between gap-1.5 mb-1">
                      <div className="flex items-center gap-1.5 min-w-0">
